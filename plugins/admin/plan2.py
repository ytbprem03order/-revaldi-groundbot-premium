  ...
  if user_id != CEO :
    ...
    try:
      ...
      if pmresults=='None':
        ...
        credit = int(fetch[5])
        module_name = "credit"
        deduct = credit + 200000
        value = deduct
        updatedata(pmid,module_name,value)
        today = str(date.today())
        validity = str(date.today()+timedelta(days=14))
        module_name = "expiry"
        value = validity
        updatedata(pmid,module_name,value)
        module_name = "status"
        value = "PREMIUM"
        updatedata(pmid,module_name,value)
        ad_resp = f"""𝗨𝗦𝗘𝗥 <a href="tg://user?id={pmid}">{pmid}</a> 𝗦𝗨𝗖𝗖𝗘𝗦𝗦𝗙𝗨𝗟𝗟𝗬 𝗣𝗨𝗥𝗖𝗛𝗔𝗦𝗘𝗗 𝗦𝗜𝗟𝗩𝗘𝗥 𝗣𝗟𝗔𝗡 𝗔𝗧 $10"""
