...
async def cmd_getplan2(Client,message):
  ...
  if user_id != CEO :
    ...
    GC1 = f"GBC-{gcgenfunc()}-{gcgenfunc()}-{gcgenfunc()}-14DAYS"
    insert_plan2(GC1)
    resp = "𝗚𝗲𝗻𝗮𝗿𝗮𝘁𝗶𝗻𝗴 𝟭"
    send = await Client.edit_message_text(message.chat.id,send.id,resp)
    GC2 = f"GBC-{gcgenfunc()}-{gcgenfunc()}-{gcgenfunc()}-14DAYS"
    insert_plan2(GC2)
    resp = "𝗚𝗲𝗻𝗮𝗿𝗮𝘁𝗶𝗻𝗴 𝟮"
    send = await Client.edit_message_text(message.chat.id,send.id,resp)
    GC3 = f"GBC-{gcgenfunc()}-{gcgenfunc()}-{gcgenfunc()}-14DAYS"
    insert_plan2(GC3)
    resp = "𝗚𝗲𝗻𝗮𝗿𝗮𝘁𝗶𝗻𝗴 𝟯"
    send = await Client.edit_message_text(message.chat.id,send.id,resp)
    GC4 = f"GBC-{gcgenfunc()}-{gcgenfunc()}-{gcgenfunc()}-14DAYS"
    insert_plan2(GC4)
    resp = "𝗚𝗲𝗻𝗮𝗿𝗮𝘁𝗶𝗻𝗴 𝟰"
    send = await Client.edit_message_text(message.chat.id,send.id,resp)
    GC5 = f"GBC-{gcgenfunc()}-{gcgenfunc()}-{gcgenfunc()}-14DAYS"
    insert_plan2(GC5)
    resp = "𝗚𝗲𝗻𝗮𝗿𝗮𝘁𝗶𝗻𝗴 𝟱"
    send = await Client.edit_message_text(message.chat.id,send.id,resp)
